...
class Package:
    def __init__(self, install_script=None, dockerfile_path=None, base_image=None):
        self.install_script = install_script
        if dockerfile_path:
            self.dockerfile = get_dockerfile_as_string(dockerfile_path)
        else:
            raise Exception('dockerfile path not available.')
        if base_image:
            self.base_image = base_image
        else:
            raise Exception('base_image path not available.')



def get_dockerfile_as_string(file_path):
    with open(file_path, 'r') as fileobject:
        stringified: str = fileobject.read()
    return stringified
...
def build(base_image, package_dirs, output_file_path):
    ...
    for dir in package_dirs:
        ...
        package = Package(installer_path, dockerfile_path, base_image)
        packages.append(package)
    generated_template = raw_template.render(packages=packages, base_image=base_image)
