...
def build_image(
    docker_client, dockerfile_name, workspace_path, image_tag, verbose=False
):
    """
    ...
    try:
        output = docker_client.api.build(
            dockerfile=dockerfile_name, path=workspace_path, tag=image_tag, rm=True
        )
        if verbose:
            for line in output:
                # print(0, line)
...
def create_intermediate_image(
    docker_client, base_image, dependencies, workspace_path, mode, verbose
):
    """
    ...
    return build_image(docker_client, image_name, workspace_path, tag, verbose)
...
def create_final_image(
    ...
    workspace_path,
    config,
    local_testing_destination=None,
    verbose=False,
):
    """
    ...
    return build_image(client, image_name, workspace_path, tag, verbose)
    # return build_image(client, image_name, workspace_path, tag, True)
...
def prepare_workspace(
    workspace_path, template_path, local_cached_dependency_source_path=None
):
    """
    ...
    if local_cached_dependency_source_path:
        shutil.copytree(
            local_cached_dependency_source_path, path.join(workspace_path, "dependency")
        )
...
def run_tests(client, container, input_lines, expected_lines):
    test_status = True
    logs = client.api.attach(
        container, stream=True, logs=True, stdout=True, stderr=True
    )
    for log in logs:
...
def main(
    ...
    if template_type == "dependency":
        ...
        template_path = path.join(
            os.getcwd(), DIR_PATH_TO_LANGUAGES, template_name, "template"
        )
        tld_path = path.join(os.getcwd(), DIR_PATH_TO_LANGUAGES, template_name)
    else:
    ...
    try:
        runtime_dirs = [
            language_specific_name,
            "{}-{}".format(language_general_name, "runtime"),
        ]
        buildtime_dirs = [
            language_specific_name,
            "{}-{}".format(language_general_name, "buildtime"),
        ]
        if dependencies:
            runtime_dirs = runtime_dirs + dependencies
            buildtime_dirs = buildtime_dirs + dependencies

        runtime_image = create_intermediate_image(
            client, base_image, runtime_dirs, WORKSPACE_PATH, "runtime", verbose
        )
        buildtime_image = create_intermediate_image(
            client, base_image, buildtime_dirs, WORKSPACE_PATH, "buildtime", verbose
        )

        config_json = path.join(
            os.getcwd(), DIR_PATH_TO_LANGUAGES, language_general_name, "config.json"
        )
        with open(config_json) as f:
            config = json.load(f)

        compile_image = create_final_image(
            client,
            buildtime_image.id,
            runtime_image.id,
            runtime_dirs,
        ...
        container = run_algorithm_container(
            client, compile_image, nvidia_support, algorithmia_api_key
        )
        ...
        if automatic_testing:
            ...
            print(
                "for an example, try passing the following curl command in a separate terminal:\n"
            )
            print(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Creates a simulation of the IPA / langserver / algorithm interface. \n"
    ...
    parser.add_argument(
        ...
        dest="language_general_name",
        help="The general name for your language, "
    ...
    parser.add_argument(
        ...
        dest="language_specific_name",
        required=True,
        help="The fully specified name of your language."
    ...
    parser.add_argument(
        ...
        dest="template_type",
        required=True,
        help="The type of template we're using, this can be either:"
    ...
    parser.add_argument(
        ...
        dest="template_name",
        required=True,
        help="The name of your template directory."
    ...
    parser.add_argument(
        ...
        action="append",
        dest="dependencies",
        help="A list builder of all non-language dependency packages that your algorithm needs."
    ...
    parser.add_argument(
        ...
        dest="cleanup",
        type=bool,
        help="A boolean variable that if set, forces us to clean up docker containers and images created by this process.",
    )
    parser.add_argument(
        "--local-dependency-src",
        dest="local_src",
        help="If using a local cached dependency for testing, is the path towards that dependency on your file system.",
    )
    parser.add_argument(
        "--local-dependency-dest",
        dest="local_dest",
        help="If using a local cached dependency for testing, is the path where the dependency will live in the compileLocal image.",
    )
    parser.add_argument(
        "--nvidia-support",
        type=bool,
        help="A boolean variable that if set, ensures that the final docker image is started with a nvidia-docker context. Requires 'nvidia-docker' to be installed.",
    )
    parser.add_argument(
        "-k",
        "--algorithmia-api-key",
        type=str,
        help="A string value that when defined, provides the ALGORITHMIA_API_KEY environment variable to the algorithm runtimme, to enable algorithm client access.",
    )
    parser.add_argument(
        "-a",
        "--automatic-testing",
        type=bool,
        help="A boolean variable that when set, runs any automatic test scripts found in the algorithm template, and provides the results to the console.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Print output of docker build commands",
    )
    ...
    if not (args.local_src and args.local_dest) and (args.local_src or args.local_dest):
        raise Exception(
            "if you're using local dependencies, src & dest must be defined."
        )
    main(
